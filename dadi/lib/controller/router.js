...
class Router {
  ...
  loadRewrites (options, done) {
    ...
    if (self.rewritesDatasource && self.loadDatasourceAsFile) {
      ...
      let rules = []
      const stream = fs.createReadStream(self.rewritesFile, {
        encoding: 'utf8'
      })
   ...
   */
  validate (route, options, req, res) {
   ...
   *
   */
  injectRequestParams (matchedRoute, keys, req) {
    req.params = {}
   ...
   *  @api public
   */
  testConstraint (route, req, res, callback) {
