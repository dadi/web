 ...
 */
class Controller {
  constructor (page, options, meta, engine, cache) {
  ...
  process (req, res, next) {
    ...
    this.loadData(req, res, data, (err, loadedData, dsResponse) => {
      ...
      view.render((err, result, unprocessed) => {
        ...
        if (data.debugView) {
          return DebugView(req, res, next, view, this)(
            null,
            result,
            unprocessed
          )
  ...
  loadEventData (events, req, res, data, done) {
  ...
  processChained (chainedDatasources, data, req, done) {
    let idx = 0
    ...
    Object.keys(chainedDatasources).forEach(chainedKey => {
      ...
      if (
        ...
        chainedDatasource.schema.datasource.paginate
      ) {
   ...
   */
  getParameterValue (object, parameter) {
    let value = null
